...
public class Grid extends GridBox {
     ...
     */
    public static final int[] BOAT_SIZES = {6,5,4,3,2};
    ...
    /**
     * Shared random reference to use for randomization of the ship placement.
     */
    private Random rand;
    /**
    ...
    public Grid(int x, int y) {
        super(x, y, CELL_SIZE*GRID_WIDTH, CELL_SIZE*GRID_HEIGHT); // we give the grid details here
        createMarkerGrid(); // create a marker grid
        ships = new ArrayList<>();
        rand = new Random();
        showShips = false;
    }
    ...
    public void paint(Graphics g) {
        for(Ship ship : ships) {
            if(showShips ||  ship.isDestroyed()) { // removed game debug
                ship.paint(g);
            }
        }
        drawMarkers(g);
     ...
     */
    public boolean markPosition(Coordinate posToMark) {
        markers[posToMark.x][posToMark.y].mark();

        allShipsDestroyed = true; // why this?
     ...
     */
    public boolean isPositionMarked(Coordinate posToTest) {
     ...
     */
    public Marker getMarkerAtPosition(Coordinate posToSelect) { // why do we do this?
     ...
     */
    public Coordinate getCoordinateInGrid(int mouseX, int mouseY) {
        if(!isPositionInside(new Coordinate(mouseX,mouseY))) return new Coordinate(-1,-1); // why -1, -1

        return new Coordinate((mouseX - position.x)/CELL_SIZE, (mouseY - position.y)/CELL_SIZE);
     ...
     */
    public boolean canPlaceShipAt(int gridX, int gridY, int segments, boolean sideways) { // can add coordinate here too
    ...
    /**
     * Creates all the marker objects setting their draw positions on the grid to initialize them.
     */
    ...
    /**
     * Clears all current ships, and then randomly places all the ships.
     * The ships will not be placed over the top of other ships. This method assumes there is
    ...
    public void placeShip(int gridX, int gridY, int segments, boolean sideways) {
        placeShip(new Ship(new Coordinate(gridX, gridY), new Coordinate(position.x+gridX*CELL_SIZE, position.y+gridY*CELL_SIZE), segments, sideways), gridX, gridY);
    }
